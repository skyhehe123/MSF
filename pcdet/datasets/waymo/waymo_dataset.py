...
from ..augmentor.data_augmentor import TTA
...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if getattr(dataset_cfg, 'USE_TTA', False):
            self.tta = TTA( logger=self.logger )
            self.use_tta = True
            self.num_augs = self.tta.num_augs
        else:
            self.tta = None
            self.use_tta = False
        ...
        self.num_frames = abs(self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0])+1
    ...
    def transform_prebox_to_current(pred_boxes3d, pose_pre, pose_cur):
        ...
        """
        assert pred_boxes3d.shape[-1] in [7, 9, 11]
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False, load_prev_gt_boxes=False):
        """
        Args:
            info:
            points:
        ...
        sample_idx_pre_list = sample_idx + np.arange(sequence_cfg.SAMPLE_OFFSET[0], sequence_cfg.SAMPLE_OFFSET[1])
        sample_idx_pre_list = np.clip(sample_idx_pre_list, 0, 0x7FFFFFFF)
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            if sequence_cfg.get('ONEHOT_TIMESTAMP', False):
                ...
                points_pre = np.hstack([points_pre, 0.1 * (idx+1) * np.ones((points_pre.shape[0], 1)).astype(points_pre.dtype)])  # one frame 0.1s
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_cfg.get('SEQUENCE_CONFIG', None) is not None and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED:
            points, num_points_all, sample_idx_pre_list, poses, pred_boxes, pred_scores, pred_labels = self.get_sequence_data(
                ...
                load_pred_boxes=self.dataset_cfg.get('USE_PREDBOX', False), load_prev_gt_boxes=False
            )
...
def create_waymo_gt_database(
    ...
    if use_parallel:
        dataset.create_groundtruth_database_parallel(
            info_path=train_filename, save_path=save_path, split='val', sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag,
...
if __name__ == '__main__':
    ...
    if args.func == 'create_waymo_infos':
        ...
        create_waymo_infos(
            ...
            data_path=Path('/data0/billyhe/Waymo'), #ROOT_DIR / 'data' / 'waymo',
            save_path=Path('/data0/billyhe/Waymo'), #ROOT_DIR / 'data' / 'waymo',
        ...
        create_waymo_gt_database(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=Path('/home/billyhe/Waymo'),
            save_path=Path('/home/billyhe/Waymo'),
            processed_data_tag=args.processed_data_tag,
