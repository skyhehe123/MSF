    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            ...
            if 'roi_boxes' in data_dict.keys():
                num_rois, dim = data_dict['roi_boxes'].shape
                ...
                data_dict['roi_boxes'] = roi_boxes.reshape(num_rois, dim)
...
class TTA(object):
    ...
    def forward(self, data_dict):
        ...
        aug_points_list = list()
        for scale in [0.95, 1.0, 1.05]:
            aug_points = points.copy()
            aug_points[:, :3] = aug_points[:, :3] * scale
            aug_points_list.append(aug_points)
            
            # aug_points_flip_x = aug_points.copy()
            # aug_points_flip_x[:, 1] = -aug_points_flip_x[:, 1]
            # aug_points_list.append(aug_points_flip_x)
           
            # aug_points_flip_y = aug_points.copy()
            # aug_points_flip_y[:, 0] = -aug_points_flip_y[:, 0]
            # aug_points_list.append(aug_points_flip_y)

            # aug_points_flip_xy = aug_points.copy()
            # aug_points_flip_xy[:, 1] = -aug_points_flip_xy[:, 1]
            # aug_points_flip_xy[:, 0] = -aug_points_flip_xy[:, 0]
            # aug_points_list.append(aug_points_flip_xy)

        data_dict['aug_points'] =  aug_points_list
